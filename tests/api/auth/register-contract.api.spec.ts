 ...
 * Purpose: observe and document the actual API contract for all scenarios.
 * Remove or skip this file once the contract is confirmed and assertions are in place.
 *
...
test.describe('CONTRACT EXPLORATION — POST /auth/provider/register', () => {
  ...
  test('scenario: successful registration', async ({ authClient }) => {
    ...
    await printResponse('Successful registration', response.status(), response.headers(), await response.text());
  ...
  test('scenario: duplicate email', async ({ authClient }) => {
    ...
    const response = await authClient.register(payload);
    await printResponse('Duplicate email', response.status(), response.headers(), await response.text());
  ...
  test('scenario: acceptedTerms false', async ({ authClient }) => {
    ...
    const response = await authClient.register(payload);
    await printResponse('acceptedTerms: false', response.status(), response.headers(), await response.text());
  ...
  test('scenario: invalid email', async ({ authClient }) => {
    ...
    const response = await authClient.register(payload);
    await printResponse('Invalid email', response.status(), response.headers(), await response.text());
  ...
  test('scenario: invalid password', async ({ authClient }) => {
    ...
    const response = await authClient.register(payload);
    await printResponse('Invalid password (too short)', response.status(), response.headers(), await response.text());
